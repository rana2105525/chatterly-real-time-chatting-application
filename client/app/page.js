...
export default function Home() {
  ...
  // NEW: Close chat on ESC key
  useEffect(() => {
    const handleEsc = (e) => {
      if (e.key === "Escape") {
        setSelectedUser(null); // Deselect current chat
  ...
  useEffect(() => {
    socket.on("recieve_message", (msg) => {
      if (selectedUser && (msg.user.name === selectedUser || msg.user.name === user.current.name)) {
        setChat((prev) => [...prev, msg]);
      } else {
        setUnreadCounts((prev) => ({
          ...prev,
          [msg.user.name]: prev[msg.user.name] ? prev[msg.user.name] + 1 : 1,
        }));
      }
    });
    ...
    socket.on("user_typing", (data) => {
      ...
      setTyping((prev) => {
        if (data.typing === false) return prev.filter((u) => u !== data.user);
    ...
    socket.on("new_user", (newUser) => {
      setAllUsers((prev) => [...new Set([...prev, newUser])]);
    });
        ...
        <div className="flex w-[90%] h-[90%] bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="w-1/4 border-r border-gray-200 p-4 bg-gray-50 flex flex-col justify-between">
            <div>
              ...
              <ul className="space-y-2">
                {allUsers.map((u, i) => {
                  ...
                  if (u === user.current.name) return null; // hide self
                  return (
                    <li
                      key={i}
                      onClick={() => selectUser(u)}
                      className={`flex items-center gap-3 p-2 rounded-md shadow-sm cursor-pointer hover:bg-gray-200 ${
                        selectedUser === u ? "bg-gray-300" : "bg-white"
                      }`}
                    >
            ...
            <button
              ...
              className="mt-4 w-full py-2 px-3 rounded-md bg-red-500 text-white font-bold hover:bg-red-600"
            >
              Logout
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex-1 p-4 overflow-y-auto">
              {selectedUser ? (
                <Chat user={user.current} chat={chat} typing={typing} activeUsers={activeUsers} />
              ) : (
